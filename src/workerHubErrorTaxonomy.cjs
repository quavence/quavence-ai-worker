...
function classifyWorkerApiError({ status, error, reason, code, message } = {}) {
...
function formatWorkerStopReasonLine({ code, http = 0, message = '' } = {}) {
...
function parseWorkerStopReasonLine(line) {
...
function inferHubStateFromWorkerSignal(line) {
...
function mapStopReasonToWorkerExitReason(code) {
...
function shouldFatalStopWorker(code) {
  return code === 'token_invalid' || code === 'hub_suspended';
}

function isTokenInvalidHubState(code) {
  return code === 'token_invalid';
}

function hubStateBlocksToken(code) {
  return isTokenInvalidHubState(code);
}

function classifyHubAccessFailureFromApi(input) {
